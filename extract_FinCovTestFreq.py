...
# Define custom time period phrases
terms_patterns = [
    nlp("12 months"),
    nlp("30 days"),
    nlp("90 days"),
    ...
    nlp("next quarter"),
     nlp("this quarter"),
    nlp("the first Financial Year"),
    nlp("the last day of such Relevant Period"),
   nlp("Relevant Period"),
    nlp("Clean-Up Period"),
    nlp("Availability Period"),
    nlp("Interest Period"),
    nlp("Quarter Date"),
    nlp("Financial Year")
...
phrase_matcher.add("FINANCIAL_PERIOD", terms_patterns)



@Language.component("add_ner_ents")
def add_ner_ents(doc):
    ...
    for match_id, start, end in matches:
        if any(token.i in seen_tokens for token in doc[start:end]):
            continue
        span = Span(doc, start, end, label="FINANCIAL_PERIOD")
        spans.append(span)
        seen_tokens.update([token.i for token in doc[start:end]])


    # Combine existing entities with our new ones
    doc.ents = tuple(list(doc.ents) + spans)
    return doc

nlp.add_pipe("add_ner_ents", last=True)
...
def leverage_flag(file_name,subsections):
    # definitions
    ...
    for item in definitions:
        ...
        flat_list = [   x  for x in definitions_list if 'ADJUSTED LEVERAGE' in x[:20].upper()]
        flat_list_mean = [x for x in flat_list if 'MEAN' in x.upper()]
        if len(flat_list_mean)>0:
            leverageterm = True
    ...
    if leverageterm:
        d = {'document_name': [file_name], 'term_name': ['leverageFlag'], 'term_value': [leverageterm],
             ...
             'term_source': ['-----------'.join(flat_list_mean)]}
...
def net_or_gross(text):
    if type(text)==str:
        if ('NET' in text.upper()) and ('GROSS' in text.upper()):
            return 'Net and Gross'
        elif 'NET' in text.upper():
            return 'Net'
        elif 'GROSS' in text.upper():
            return 'Gross'
        else:
            return 'Unknown'
    else:
...
#     # Adjusted Ebitda:
#     financial_cov = [x for x in sections if ('FINANCIAL COVENANTS' in x[0].upper())]
...
#     temp_df1 = leverage_flag(file_name,subsections)
#     master_df = pd.concat([master_df,temp_df1])

# master_df=master_df.reset_index()
# master_df['Net_or_gross'] = master_df['term_source'].apply(lambda x: net_or_gross(x))
...
financial_test = [x[1] for x in subsections if 'TEST' in x[0].upper()]
financial_test_str = '\n\n'.join(financial_test)
