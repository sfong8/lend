...
for file_name in os.listdir('./markdown/'):
    ...
    debt_service = [x for x in temp_matches if 'PERMITTED' in x[0].upper()]
...
def slim_down(data):
    ...
    for key, value in data[1]:
        if key not in definitions:
            ...
            if len(value) > len(definitions[key]):
                definitions[key] = value
...
for item in test_lsit:
    temp_dict = slim_down(item)
    updated_list.append((item[0],temp_dict))


test = updated_list[0][1]['Permitted Disposal']
master_df = pd.DataFrame()
for item in updated_list:
    df = pd.DataFrame.from_dict(item[1], orient='index', columns=['Definition']).reset_index()
    df.columns = ['term_value','term_source']
    df['file_name'] = item[0]
...
master_df['Montary_value'] = master_df['term_source'].apply(lambda x: extract_monetary_value(x))
